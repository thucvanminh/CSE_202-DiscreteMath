...
public class EICONP3 {
	...
	public static void main(String[] args) throws IOException {
		...
		Vertex[] graphs = readGraph(nVertices, nEdges);
		for (Vertex x : graphs) {
			if (x.visited == false) {
				bfs(x, graphs);
			}
	...
	static void bfs(Vertex v, Vertex[] graph) {
		...
		int smallest = v.id;
		int countVertice = 1;
		int countDegree = v.adjecentVertices.size();
		...
		while (!q.isEmpty()) {
			...
			for (Vertex x : current.adjecentVertices) {
				if (!x.visited) {
					x.visited = true;
					countVertice++;
					countDegree += x.adjecentVertices.size();
					if (x.id < smallest) {
						smallest = x.id;
					}
					q.add(x);
		...
		System.out.println(smallest + " " + countVertice + " " + countDegree / 2);
