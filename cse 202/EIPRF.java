...
public class EIPRF {
        ...
        public static void main(String[] args) throws IOException {
            ...
            List<Vertex>  compoVertices = new ArrayList<Vertex>();
            dfs(graph[0], compoVertices);
            for (Vertex v: compoVertices) {
                sb.append(v.id+" ");
            }
            System.out.println(sb);
        }

        static boolean dfs(Vertex v,List<Vertex> compoVertices){
            v.visited = true;
            compoVertices.add(v);
            for(Vertex w: v.adjacentVertices){
                if(w.visited == false){
                    if(dfs(w, compoVertices) == true){
                        return true;
                    }
                }
                else{
            ...
            compoVertices.remove(compoVertices.size()-1);
            return false;
        }
